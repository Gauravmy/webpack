...
class ConsumeSharedModule extends Module {
	...
	getExportsType(moduleGraph, strict) {
		// Populate `buildMeta` from fallback module
		if (
			this.options.import &&
			this.buildMeta &&
			!("exportsType" in this.buildMeta)
		) {
			const fallbackModule = this._getFallbackModule(moduleGraph);
			if (fallbackModule && fallbackModule.buildMeta) {
				this.buildMeta = { ...fallbackModule.buildMeta };
			}
		}

		return super.getExportsType(moduleGraph, strict);
	}
